...
$pdo_verifica = $conexao_pdo->prepare("select status from aluno");
...
$pdo_verifica = $conexao_pdo->prepare("select * from aluno WHERE (status = 1 OR status = 0)  order by cr desc");
                     $pdo_verifica->execute();
            while($fetch = $pdo_verifica->fetch()){
